...
import { generateCashBillPdf } from '../cashBillPdf';
...
describe('generateCashBillPdf', () => {
  it('returns a Blob', () => {
    const result = generateCashBillPdf(mockPatient, mockMedicines, mockSettings);
    expect(result).toBeInstanceOf(Blob);
  });

  it('calls the jsPDF constructor (verified via output)', () => {
    generateCashBillPdf(mockPatient, mockMedicines, mockSettings);
  ...
  it('handles empty medicines array', () => {
    const result = generateCashBillPdf(mockPatient, [], mockSettings);
    expect(result).toBeInstanceOf(Blob);
    expect(mockDocRef.current.addPage).not.toHaveBeenCalled();
  });
  ...
  it('handles missing patient fields', () => {
    ...
    const result = generateCashBillPdf(minimalPatient, mockMedicines, mockSettings);
    expect(result).toBeInstanceOf(Blob);
  });
  ...
  it('adds page when signature block overflows', () => {
    (autoTable as any).mockImplementation((doc: any) => {
      doc.lastAutoTable = { finalY: 222 };
    });
    generateCashBillPdf(mockPatient, mockMedicines, mockSettings);
    expect(mockDocRef.current.addPage).toHaveBeenCalled();
  });
  ...
  it('uses doctor seal when available', () => {
    ...
    generateCashBillPdf(mockPatient, mockMedicines, settingsWithSeal);
