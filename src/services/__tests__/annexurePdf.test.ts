...
import { generateAnnexurePdf } from '../annexurePdf';
...
describe('generateAnnexurePdf', () => {
  it('returns a Blob', () => {
    const result = generateAnnexurePdf(mockPatient, mockMedicines, mockSettings);
    expect(result).toBeInstanceOf(Blob);
  });

  it('calls the jsPDF constructor (verified via output)', () => {
    generateAnnexurePdf(mockPatient, mockMedicines, mockSettings);
    expect(mockDocRef.current.output).toHaveBeenCalledWith('blob');
  });

  it('renders annexure title', () => {
    generateAnnexurePdf(mockPatient, mockMedicines, mockSettings);
    expect(mockDocRef.current.text).toHaveBeenCalledWith('ANNEXURE-1', expect.any(Number), expect.any(Number), expect.objectContaining({ align: 'center' }));
    expect(mockDocRef.current.text).toHaveBeenCalledWith('Prescribed Medicine Details & Dosage Schedule', expect.any(Number), expect.any(Number));
  });

  it('handles empty medicines array', () => {
    const result = generateAnnexurePdf(mockPatient, [], mockSettings);
    expect(result).toBeInstanceOf(Blob);
  });
  ...
  it('handles missing patient fields', () => {
    ...
    const result = generateAnnexurePdf(minimalPatient, mockMedicines, mockSettings);
    expect(result).toBeInstanceOf(Blob);
  });

  it('adds a new page when content overflows', () => {
    (autoTable as any).mockImplementation((doc: any) => {
      doc.lastAutoTable = { finalY: 280 };
    });
    generateAnnexurePdf(mockPatient, mockMedicines, mockSettings);
  ...
  it('uses doctor seal when available', () => {
    ...
    generateAnnexurePdf(mockPatient, mockMedicines, settingsWithSeal);
